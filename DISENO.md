...
Cada volumen tiene, además de su nombre (derivado del archivo), un **"valor" editable** que es el que efectivamente se usa como sufijo de su columna `dentro__<valor>` y como texto en la columna resumen — en vez de forzar a que ese valor coincida con el nombre del archivo. Esto permite, por ejemplo, cargar `fase_2025.dxf`, `fase_2026.dxf`, `fase_2027.dxf` y asignarles como valor simplemente `2025`, `2026`, `2027`, de forma que la columna resumen quede directamente con el período/fase al que pertenece cada bloque, sin depender de la convención de nombres de los archivos.
...
### 13.6 Límite de memoria con modelos de bloques muy grandes (probado por el usuario con ~400MB)

Al probar Modo 4 con un modelo de bloques real de ~400MB contra los dos volúmenes de `test_MINSUR`, la pestaña del navegador quedó en blanco (síntoma típico de que el proceso del navegador se quedó sin memoria). Causa más probable, identificada por revisión de código (no reproducible en este entorno por la caída del sandbox de `bash`):

1. `FLG.parseCSV` (usado por los 4 modos para cargar cualquier tabla) leía el archivo completo como un único string y le aplicaba **tres `.replace()` encadenados sobre todo el archivo** más `.split('\n')` y `.filter(...)` — cada uno de esos pasos construye una copia nueva del tamaño completo del archivo o un arreglo con una entrada por línea, todos vivos en memoria al mismo tiempo, antes de llegar siquiera a construir las filas parseadas.
2. El handler de "Clasificar bloques" de Modo 4 construía el resultado con `rows.map(r => Object.assign({}, r))`, es decir, **duplicaba por completo** el arreglo de filas ya parseado (que para un modelo de 400MB con millones de filas ya es la estructura más pesada en memoria) justo en el momento de mayor uso.

**Mitigación aplicada** (sin poder verificarla end-to-end por la caída del sandbox): se reescribió `parseCSV` para recorrer el archivo línea por línea con `indexOf`/`slice` en vez de `replace`/`split`/`filter` sobre el string completo — `slice()` sobre un string grande no copia su contenido en V8 (usa una "sliced string" que referencia al original), lo que evita esas copias transitorias. En Modo 4, el resultado ahora se construye **mutando directamente** el arreglo de filas ya cargado (agregando las columnas nuevas in-place) en vez de duplicarlo, y se reemplazó el arreglo auxiliar `perBlockNames` (un array por bloque) por un `Uint8Array` de máscara de validez más concatenación directa de strings.

Esto debería reducir sustancialmente el pico de memoria tanto en la carga del archivo como en la clasificación, pero **no hay garantía de que sea suficiente para 400MB** — la estructura final (un objeto JS por fila, con todas sus columnas como strings) sigue siendo inherentemente pesada a esa escala; una solución más robusta (almacenamiento columnar con arrays tipados, parseo en Web Worker por trozos como el DXF) implicaría un cambio de arquitectura mayor, no hecho en esta sesión. **Pendiente: que el usuario reintente con el mismo archivo de 400MB y confirme si el problema se resolvió, se redujo (llegó más lejos antes de fallar), o persiste igual** — de eso depende si vale la pena una reescritura más profunda del parser tabular.

### 13.7 Pantalla en blanco al exportar 2.6M bloques ya clasificados

Al exportar el resultado (XLSX o CSV) de un modelo de 2.6 millones de bloques ya clasificado, la pantalla también quedaba en blanco. Causas identificadas por revisión de código:

1. `buildExportRowsVol()` duplicaba **otra vez completo** el arreglo de resultados (`state4.results.map(...)`, creando un objeto nuevo por fila) justo después de que la clasificación ya había dejado la memoria ajustada — mismo patrón de sobrecopia que en 13.6, pero en el paso de exportar.
2. Para XLSX específicamente, 2.6M filas **supera el límite físico de una hoja de Excel** (1,048,576 filas, encabezado incluido) — `SheetJS` intentaría construir igual un objeto de hoja con más celdas de las que el formato admite, lo que además de ser inválido es extremadamente pesado en memoria (un objeto por celda).
3. La exportación CSV armaba un arreglo `lines` con todas las filas y después hacía `lines.join('\n')`, construyendo **un único string contiguo** del tamaño completo del CSV (varios cientos de MB) — una asignación de memoria grande y monolítica, más propensa a fallar que la misma cantidad de memoria repartida en piezas más chicas.

**Corrección aplicada:** ambas exportaciones ahora leen directamente `state4.results` (sin duplicar el arreglo). El botón de Excel valida primero si el modelo excede las 1,048,575 filas de datos permitidas y, si las excede, no intenta generar el archivo — muestra un aviso y sugiere usar CSV, que no tiene ese límite. La exportación CSV arma las líneas como piezas separadas de un arreglo y las pasa directo a `new Blob(partes, {...})`, sin unirlas antes en un string único — un `Blob` puede recibir muchas piezas sin que el motor JS necesite reservar de una sola vez un bloque contiguo del tamaño completo del archivo.

Mismo patrón de duplicación (`buildExportRows*` + `lines.join('\n')`) sigue presente, sin corregir aún, en las exportaciones de Modo 1, Modo 2 y Modo 3 — no reportado como problema todavía en esos modos, pero con el mismo riesgo si se usan con bases igualmente grandes.

**Seguimiento — el fix anterior no fue suficiente:** con un modelo real de 2.6M bloques, la exportación CSV volvió a dejar la pantalla en blanco incluso después del cambio de 13.7. La razón: aunque ya no se duplicaba `state4.results`, seguía armándose un arreglo `parts` con **una string por cada una de las 2.6M filas**, todas vivas en memoria a la vez hasta el `new Blob(parts)` final — en la práctica, casi tanta memoria adicional como la que ya ocupaba el modelo de bloques cargado. Quitar el `.join('\n')` final evitó la asignación de un string único y contiguo, pero no evitó tener 2.6M strings sueltas conviviendo al mismo tiempo.

**Corrección definitiva:** el CSV ahora se construye **en lotes de 20,000 filas** (`CHUNK_ROWS`): cada lote se convierte de inmediato en su propio `Blob` (que el navegador guarda como bytes, no como strings de JS) y las strings de ese lote quedan libres para el recolector de basura antes de armar el siguiente lote. Al final se combinan ~130 Blobs de lote en uno solo (`new Blob(blobParts)`), lo que no requiere volver a copiar todo el texto en un string de JS. Además, los botones de exportar (Excel y CSV) ahora quedan deshabilitados con el texto "Generando…" mientras dura la exportación (usando `runBusy`, que difiere el trabajo pesado con `setTimeout` para que el navegador llegue a pintar ese estado antes de la parte síncrona), y la URL del blob se revoca varios segundos después del clic en vez de inmediatamente, para no cortar la descarga de un archivo grande a medio escribir.

### 13.8 Filtro de exportación: solo bloques dentro de algún volumen

A pedido del usuario (que solo necesita las filas positivas para cruzarlas después en R), se agregó un checkbox en la card de parámetros ("Exportar solo los bloques que quedaron dentro de algún volumen") que reduce el archivo de salida a solo esos bloques, sin tocar la tabla ni el resumen en pantalla (que siguen mostrando el modelo completo).

El filtro (`getExportRowsVol`) no depende del texto de la columna resumen ni de "noneLabel" (ambos configurables por el usuario y por lo tanto frágiles como criterio de filtrado); en su lugar, un bloque se considera "dentro de algún volumen" si al menos una de sus columnas `dentro__<valor>` quedó en `'sí'`. Como `Array.prototype.filter` sobre un arreglo de objetos solo copia las referencias que pasan el filtro (no los objetos apuntados), este paso es económico en memoria incluso si el modelo tiene millones de filas y el filtro no descarta casi nada. Este filtro, además de resolver el pedido original, mitiga por otra vía el problema de memoria de 13.6-13.7: si la mayoría de los bloques queda "fuera" (caso típico), el volumen de datos a exportar puede reducirse en varios órdenes de magnitud.
